...
class WSADPHYSICS_API UWSADCharacterMovementComponent : public UCharacterMovementComponent
{
	...
	UFUNCTION(Unreliable, Server, WithValidation)
	void ServerSetDeltaRotation(const FQuat& DeltaRotation);
...
public:
	...
	UFUNCTION(BlueprintCallable) void SetDeltaRotation(const FQuat& DeltaRotation);
};
